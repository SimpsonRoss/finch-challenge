...
export async function GET(
  ...
  const res = await fetch('https://api.tryfinch.com/employer/individual', {
    method: 'POST',
    headers: {
  ...
  if (res.status === 501) {
    return NextResponse.json(
      { error: 'This provider does not support individual data.' },
      { status: 501 }
    );
  }  
  ...
  if (!res.ok) {
    ...
    console.error(`Finch /individual error (${res.status}):`, errorText);
